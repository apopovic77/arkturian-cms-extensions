...
6. Validate the site at `https://example.com` (if applicable) and confirm fresh assets reached `/var/www/arkturian-cms-extensions`.

## Rollback Strategy
- Use `.devops/rollback.sh` on the server to restore from `/var/backups/arkturian-cms-extensions-*`.
